...
$query = "SELECT utenti.nome as nome, utenti.email as email, indirizzi.via, indirizzi.citta, indirizzi.provincia, indirizzi.cap, intestatario_carta, numero_carta, scadenza_carta 
 FROM utenti INNER JOIN indirizzi ON indirizzi.utente_id = utenti.id INNER JOIN metodipagamento ON utenti.id = metodipagamento.utente_id WHERE utenti.id=$id LIMIT 1";
global $dbh;
